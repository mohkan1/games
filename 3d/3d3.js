...
function create ()
{
    gameState.keys = this.input.keyboard.addKeys('W,D,S,A');
    this.add.image(300,100, 'sky').setScale(2);
    gameState.player = this.physics.add.sprite(225, 270, 'dude');
    gameState.player.setCollideWorldBounds(true);
    gameState.cursors = this.input.keyboard.createCursorKeys();

    gameState.heart = this.add.image(450, 20, 'heart');
    gameState.heart2 = this.add.image(500, 20, 'heart');
    gameState.heart3 = this.add.image(550, 20, 'heart');
    gameState.life = 3;
    ...
    function makeBugsBigger(bug) {
        var number = 0.5;
        gameState.makeBugsBiggerTimer = setInterval(function(){ 
            bug.setScale(number);
            number += 0.01;
         }, 20);
    ...
    function makeBonusBigger(bonusElement) {
        var number = 0.5;
        gameState.makeBonusBiggerTimer = setInterval(function(){ 
...
function update ()
{
    ...
    if (gameState.cursors.right.isDown || gameState.keys.D.isDown) {
        ...
        gameState.player.setScale(makingPlayerSmaller);
        makingPlayerSmaller -= 0.005;
    ...
    switch (gameState.life) {
        ...
        gameState.scoreText.setText(`Game Over! click to start again`);
        game.paused = true;
        ...
        clearInterval(gameState.makeBugsBiggerTimer);
        clearInterval(gameState.makeBonusBiggerTimer);
    ...
    if (gameState.score >= 500) {
        gameState.scoreText.setText(`Cong! Click to start again`);
        game.paused = true;
